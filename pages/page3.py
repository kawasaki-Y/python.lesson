...
# ページ設定
st.set_page_config(page_title="財務計画とシミュレーション", page_icon="📉", layout="wide")
...
growth_rate = st.sidebar.slider("年間成長率 (%)", min_value=0, max_value=50, value=10)
...
st.sidebar.header("事業ごとの年間売上・コスト")
business_data = []
for i in range(num_businesses):
    ...
    business_data.append({"事業": f"事業 {i+1}", "売上": revenue, "コスト": cost})
...
for year in range(1, simulation_years + 1):
    ...
    for business in business_data:
        annual_revenue = business["売上"] * (1 + growth_rate / 100) ** (year - 1)
        annual_cost = business["コスト"] * (1 + growth_rate / 100) ** (year - 1)
